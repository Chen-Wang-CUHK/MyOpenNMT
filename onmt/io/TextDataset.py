...
# ------------------ my TextDataset class ----------------------
class MyTextDataset(ONMTDatasetBase):
    """ Dataset for data_type=='text'
        ...
        Args:
            ...
            src_examples_iter (dict iter): preprocessed source example
                dictionary iterator.
            ans_examples_iter (dict iter): preprocessed answer example
                dictionary iterator.
            tgt_examples_iter (dict iter): preprocessed target example
                dictionary iterator.
            num_src_feats (int): number of source side features.
            num_tgt_feats (int): number of target side features.
            src_seq_length (int): maximum source sequence length.
            ans_seq_length (int): maximum answer sequence length.
            tgt_seq_length (int): maximum target sequence length.
            dynamic_dict (bool): create dynamic dictionaries?
            use_filter_pred (bool): use a custom filter predicate to filter
                out examples?
    ...
    def __init__(self, fields, src_examples_iter, ans_examples_iter, tgt_examples_iter,
                 num_src_feats=0, num_ans_feats=0, num_tgt_feats=0,
        ...
        # the src and tgt features and alignment information.
        if tgt_examples_iter is not None:
            examples_iter = (self._join_dicts(src, ans, tgt) for src, ans, tgt in
                             zip(src_examples_iter, ans_examples_iter, tgt_examples_iter))
        else:
    ...
    def make_text_examples_nfeats_tpl(path, truncate, side):
        ...
        # to get the num_feats.
        examples_nfeats_iter = \
            MyTextDataset.read_text_file(path, truncate, side)
    ...
    def read_text_file(path, truncate, side):
        ...
        with codecs.open(path, "r", "utf-8") as corpus_file:
            for i, line in enumerate(corpus_file):
                ...
                words, feats, n_feats = \
                    MyTextDataset.extract_text_features(line)
    ...
    def get_fields(n_src_features, n_tgt_features):
        ...
        # add for ans
        fields["ans"] = torchtext.data.Field(
            pad_token=PAD_WORD,
            include_lengths=True)
        # add for ans
        for j in range(n_src_features):
    ...
    def get_num_features(corpus_file, side):
        ...
        with codecs.open(corpus_file, "r", "utf-8") as cf:
            f_line = cf.readline().strip().split()
            _, _, num_feats = MyTextDataset.extract_text_features(f_line)
